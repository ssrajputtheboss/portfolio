import { Box, IconButton, Icon, Tooltip, Text, Flex, Input } from '@chakra-ui/react';
...
export default function TerminalUI({ getCommand }: { getCommand: Function }): React.ReactElement {
  ...
  if (!isTerminalOpen) {
    return (
      <Tooltip hasArrow label='Open Terminal' bgColor='blackAlpha.900' textColor='white'>
        <IconButton
          _hover={{ bgColor: 'blue' }}
          position='fixed'
          bottom={{ base: '8', lg: '20' }}
  ...
  return (
    <Box w='full' position='fixed' bottom='10' h='40'>
      <TerminalTitle
        onClose={() => {
      ...
      <Box
        ...
        <Flex flexDir='column' justifyContent='start' alignItems='start'>
          {lastText.split('\n').map((v, i) => {
            return (
              <Flex flexDir='row' justifyContent='start' alignItems='center' key={i}>
                <Text px='1' textColor='green.400'>
                  {v.substring(0, 2)}
                </Text>
                <Text px='1' textColor='white'>
              ...
              if (e.keyCode === 13) {
                ...
                if (e.currentTarget.parentElement) {
                  if (e.currentTarget.parentElement.parentElement) {
                    e.currentTarget.parentElement.parentElement.scrollTop =
