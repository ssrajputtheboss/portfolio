      ...
      <Flex flexDir='row' justifyContent='space-between' alignItems='start'>
        <Box w='full'>
          <Flex
            ...
            <Flex flexDir='column' justifyContent='space-between' alignItems='start' w='full'>
              <CardHeading title={props.header} langs={props.langs} />
              <Text p='4' fontSize='sm'>
                {' '}
                {props.description}{' '}
              </Text>
              <Box display={isOpen ? 'block' : 'none'} p='4'>
                <ReadMe src={props.url} />
              </Box>
              <LiveProjectLink isLive={props.isLive} projectLink={props.projectLink} />
            </Flex>
            <Donut
              series={props.langs.map(({ data }) => {
                return data;
              })}
              labels={props.langs.map(({ name }) => {
                return name;
              })}
            />
          </Flex>
