...
export const fetchReadMeFromURL: Function = async (url: string): Promise<string> => {
  const res: string | Response | (() => Promise<string>) = await fetch(url).then((res) =>
    res.text()
  );
  if (_.isString(res)) {
    return res;
        ...
        if (list[k].match(reCode)) {
          ...
          if (
            ...
            obj.set(lastHeader, {
              title: obj.get(lastHeader)?.title || '',
              content:
                obj.get(lastHeader)?.content +
                  `<a href="${url}" style="color:blue;">${' ' + title + ' '}</a>` ||
                `<a href="${url}">${title}</a>`
            });
          }
        }
